...
export class MultisigAccountModificationTransactionBodyBuilder {
    ...
    public static loadFromBinary(payload: Uint8Array): MultisigAccountModificationTransactionBodyBuilder {
        ...
        // tslint:disable-next-line: max-line-length
        for (let i = 0; i < (Array.isArray(publicKeyAdditionsCount) ? GeneratorUtils.compact(publicKeyAdditionsCount) : publicKeyAdditionsCount); i++) {
        ...
        // tslint:disable-next-line: max-line-length
        for (let i = 0; i < (Array.isArray(publicKeyDeletionsCount) ? GeneratorUtils.compact(publicKeyDeletionsCount) : publicKeyDeletionsCount); i++) {
            const item = KeyDto.loadFromBinary(Uint8Array.from(byteArray));
